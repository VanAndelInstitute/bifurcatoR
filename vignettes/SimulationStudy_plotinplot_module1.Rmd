...
all.res = readRDS("~/bbc-secondary/research/POSA_20230314_TR01Shiny_VBCS-718/simulation_results/module1_n_w_7_tests_sim(1000).rds")
...
p1 = ggplot(module1[module1$Prop==p &module1$Distribution == dist &module1$Combos == combo,], aes(SampleSize, power,
    ...
    geom_line(linewidth=1,position = position_dodge2(width = .5)) +
    # stat_smooth(linewidth=1,position = position_dodge2(width = .5),se=F) +
    # facet_grid(mu2 ~ sd2,
    #            labeller = labeller(mu2 = mu2_labels,
    #                                sd2 = sd2_labels))+
    theme_minimal(16)+
    scale_color_manual(values=unname(mypal))+
    scale_size_manual(values=c(3,1.5,0.1))+
  # scale_size_continuous(breaks=c(0.01,0.05,0.1,0.5,1),range=c(3,0),limits=c(0.00001,1))+
    # ggtitle(paste0("Mixing Proportion =", p))+
    ylab("Power") + xlab("Sample size")+
  scale_y_continuous(limits=c(0,1),breaks=seq(0,1,0.2))+scale_x_continuous(breaks=c(0,20,50,100,200,400),trans="log2")
   
  leg <- ggpubr::get_legend(p1+theme(legend.position="top",legend.box = "vertical"))
  # ggdraw(leg)
...
# args = c(shape = mixdist::weibullpar(df_norm$mu1[1],df_norm$sd1[1], loc = 0)$shape, scale = mixdist::weibullpar(df_norm$mu1[1],df_norm$sd1[1], loc = 0)$scale),aes(color="Reference Group"),alpha=0.5,linewidth=0.7) + 
  ...
  p2_leg = ggpubr::get_legend(p2+theme(legend.position = "top",legend.box = "horizontal"))
...
for(p in unique(all.res[all.res$Test %in% var_tests,]$Prop)){
 ...
 # com_plots[[as.character(p)]] = plots[[1]] + xlab("") +
 #                                plots[[2]] + xlab("") +ylab("") +
 #                                plots[[3]] + xlab("") +ylab("") +
 #                                plots[[4]] + xlab("") +
 #                                plots[[5]] + xlab("") +ylab("") +
 ...
 #                                plots[[7]] + xlab("") +
 #                                plots[[8]] + xlab("") +ylab("") +
 #                                plots[[9]] + xlab("") +ylab("")  + plot_annotation(
 #  title = paste0("Sampling ratio ", ifelse(p<0.5, gsub("/",":",MASS::fractions( p/(1-p))),paste0(p/(1-p),":1")))) &  theme(plot.title = element_text(hjust = 0.5))
...
tiff(file = "~/bbc-secondary/research/POSA_20230314_TR01Shiny_VBCS-718/simulation_results/module1_var.tif",height = 6200,width=4200 ,res=200 )
plot_grid(pn,pw,scale=  c(0.95,0.95),ncol=2,rel_widths = c(1,1))
dev.off()
...
all.res$Test[all.res$Test == "Permutations (Raw)"] = "Permutation test of the mean"
all.res$Test[all.res$Test == "Non-Parametric ANOVA"] = "Ranked ANOVA"

var_tests = c("ANOVA", "Ranked ANOVA" ,"Permutation test of the mean")
com_plots = list()
for(p in unique(all.res[all.res$Test %in% var_tests,]$Prop)){
  plots = lapply(levels(all.res$Combos),function(z) plot_both("norm",p,all.res[all.res$Test %in% var_tests,],z)[[1]])
...
tiff(file = "~/bbc-secondary/research/POSA_20230314_TR01Shiny_VBCS-718/simulation_results/module1_mean.tif",height = 6200,width=4200 ,res=200 )
plot_grid(pn,pw,scale=  c(0.95,0.95),ncol=2,rel_widths = c(1,1))
dev.off()
