...
sim_mean <- function(i){
  ...
  if(params.sim$dists[i] == "weibull"){ all.tests <- c("mclust_E", "mclust_V" , "mt","dip", "WmixR","NmixR","LNmixR","ACR")
  } else if(params.sim$dists[i] == "gamma") { all.tests <- c("mclust_E", "mclust_V" , "mt","dip","NmixR","GmixR","LNmixR","ACR")
  } else {
    all.tests <- c("mclust_E", "mclust_V" , "mt","dip", "NmixR","LNmixR","ACR")
    }
  ...
  write.csv(file = paste0("/varidata/research/projects/bbc/research/POSA_20230314_TR01Shiny_VBCS-718/bifurcatoR_main/sims/bimodal/2000_bimodal_grid_sim",i,".csv"),
                data.frame(sims,
...
# mean.sims <- mclapply(830:840,function(x) sim_mean(x) , mc.cores = 12 )
