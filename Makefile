...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "bifurcatoR", "tests")); system.time(test_check("bifurcatoR", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran bifurcatoR_$(VERSION).tar.gz
	rm -rf bifurcatoR.Rcheck/
